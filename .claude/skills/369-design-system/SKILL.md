...
| "Cleveland-McGill perceptual accuracy ladder? Mackinlay APT? Wilkinson Grammar of Graphics? Why position > color?" | `references/canon/cleveland-mcgill.md` — data-viz theoretical canon with measured error rates |
| "Christopher Alexander's pattern language? QWAN? 15 properties of living structure? GoF software lineage?" | `references/canon/pattern-language.md` — the framework that produced design patterns + agile + atomic design |
| "Norman's 7 principles? Krug's 3 laws? Gestalt principles? Fitts's Law? Hick-Hyman? GOMS?" | `references/canon/hci-foundations.md` — foundational HCI canon 369 owns natively |
| "Why ×3 not ×8? Müller-Brockmann Swiss grid? Modular scale? Bringhurst? Mathematical defense of Rule 1?" | `references/canon/grid-typography.md` — Swiss grid + harmonic ratios + the ×3 evidence base |
| "Inclusive design? Universal Design's 7 principles? Curb-cut effect? WCAG POUR? Accessibility as innovation?" | `references/canon/inclusive-design.md` — accessibility-as-discipline reframe (Holmes, Mace, Microsoft) |
| "Shannon entropy? Hick-Hyman log₂? Miller's 7±2 / Cowan's 4±1? Sweller cognitive load? Information capacity of a screen?" | `references/canon/information-theory.md` — the math under 369's density rules |
| "What killed Metro UI? Why did Apple HIG survive 39 years? Material Design's evolution? Bootstrap's decline? 369 positioning?" | `references/canon/design-system-history.md` — comparative survival case studies |
| "How do habits form in interfaces? Fogg's B=MAP? Hook Model? 66-day curve? Designing for the 10,000th user?" | `references/canon/habit-formation.md` — behavioral psychology of habit formation, applied to 369 |
| "Why does 369 use Unicode glyphs instead of icon libraries? Picture superiority? Bouba/Kiki? Susan Kare's principles?" | `references/canon/glyph-cognition.md` — the cognitive science of glyph choice |
| "The file format IS the interface — README.md / .NFO / dotfiles / JSON / Markdown / TOML / agent-readable structure?" | `references/canon/format-as-interface.md` — text file as longest-living UI, including AI-agent era |
